...
twin_context_t *twin_vnc_init(int width, int height)
{
    ...
    tx->fb_pool = nvnc_fb_pool_new(width, height, DRM_FORMAT_XRGB8888, 4);
    if (!tx->fb_pool)
        goto bail_display;
    ...
    tx->framebuffer = malloc(width * height * sizeof(uint32_t) * 4);
    if (!tx->framebuffer)
        goto bail_screen;
    memset(tx->framebuffer, 0, width * height * sizeof(uint32_t) * 4);

    tx->current_fb = nvnc_fb_from_buffer(tx->framebuffer, width, height,
        DRM_FORMAT_XRGB8888, width * 4);
    if (!tx->current_fb)
        goto bail_framebuffer;

    pixman_region_init(&tx->damage_region);
